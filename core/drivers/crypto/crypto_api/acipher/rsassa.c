					 ...
					 struct cryptobuf *EM)
{
	...
	*buf++ =
		(uint8_t)(0x08 + hash_oid->asn1_length + ssa_data->digest_size);
...
static TEE_Result rsassa_pkcs1_v1_5_sign(struct drvcrypt_rsa_ssa *ssa_data)
{
	...
	if (rsa) {
		...
		rsa_data.rsa_id = RSASSA_PKCS_V1_5;
...
static TEE_Result emsa_pss_encode(struct drvcrypt_rsa_ssa *ssa_data,
				  size_t emBits, struct cryptobuf *EM)
{
	TEE_Result ret = TEE_ERROR_GENERIC;
	struct drvcrypt_rsa_mgf mgf_data = {};
	struct cryptobuf hash = {};
	struct cryptobuf dbMask = {};
	struct cryptobuf DB = {};
	size_t db_size = 0;
	size_t ps_size = 0;
	size_t msg_size = 0;
